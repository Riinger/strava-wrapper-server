...
public class LoggingInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
//		MDC.put("REQID", UUID.randomUUID());
//		if ( simProperties.isVerbose() ) {
...
//		} else {
			log.info("===> Processing, HTTP request : URI = '{}'", request.getRequestURI());
//		}
...
//		} else {
			log.info("<=== Processed, HTTP response : HTTP status =  {}", response.getStatus());
//		}
		MDC.remove("REQID");
	}
}
