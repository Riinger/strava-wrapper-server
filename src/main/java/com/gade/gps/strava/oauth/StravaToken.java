...
public class StravaToken {
	...
	void init() throws IOException {
		...
		this.setAccessToken(authProperties.getProperty(ACCESS_TOKEN, ""));
		this.setRefreshToken(authProperties.getProperty(REFRESH_TOKEN, ""));
		this.setExpiresAt(Long.parseLong(authProperties.getProperty(EXPIRES_AT, "0")));
	}
	...
	public void rewrite(StravaToken newToken) throws IOException {
		...
		this.tokenType = newToken.tokenType;
		Properties authProperties = new Properties();
		authProperties.setProperty(REFRESH_TOKEN, this.getRefreshToken());
		authProperties.setProperty(ACCESS_TOKEN, this.getAccessToken());
		authProperties.setProperty(EXPIRES_AT, String.valueOf(this.getExpiresAt()));
		LocalDateTime expiresAtDateTime = LocalDateTime.ofEpochSecond(this.getExpiresAt(), 0, OffsetDateTime.now().getOffset());
